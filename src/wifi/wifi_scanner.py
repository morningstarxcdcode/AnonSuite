...
class WiFiScanner:
    ...
    def check_tools(self) -> Dict:
        ...
        try:
            result = subprocess.run(
                ["which", "iwconfig"], capture_output=True, timeout=5
            )
        ...
        try:
            result = subprocess.run(
                ["which", "system_profiler"], capture_output=True, timeout=5
            )
                ...
                if not interfaces:
                    ...
                    for iface_name in common_macos_interfaces:
                        try:
                            ...
                            if (
                                result.returncode == 0
                                and "status: active" in result.stdout
                            ):
                                interfaces.append(
                                    {
                                        "name": iface_name,
    ...
    def scan_networks(
        self, interface: Optional[str] = None, timeout: int = 30
    ) -> List[Dict]:
            ...
            if result.returncode == 0:
                ...
                for item in data.get("SPAirPortDataType", []):
                    ...
                    for iface_data in interfaces_data:
                        ...
                        current_network = iface_data.get(
                            "spairport_current_network_information", {}
                        )
                        if current_network:
                            network = {
                                "ssid": current_network.get(
                                    "spairport_network_name", "Unknown"
                                ),
                                "bssid": current_network.get(
                                    "spairport_bssid", "Unknown"
                                ),
                                "channel": current_network.get("spairport_channel", 0),
                                "signal_level": self._parse_signal_strength(
                                    current_network.get("spairport_signal_noise", "")
                                ),
                                "encryption": self._parse_macos_security(
                                    current_network.get("spairport_security", "Unknown")
                                ),
                                "frequency": "",
                                "quality": "",
                                "cipher": "",
    ...
    def _scan_macos_airport(self, timeout: int = 30) -> List[Dict]:
        ...
        try:
            ...
            if not Path(airport_path).exists():
                self.logger.warning(
                    "macOS airport utility not found, trying alternative methods"
                )
                return self._scan_macos_alternative()

            # Use airport utility to scan
            result = subprocess.run(
                [airport_path, "-s"], capture_output=True, text=True, timeout=timeout
            )
                    ...
                    try:
                        ...
                        if len(parts) >= 6:
                            ...
                            network = {
                                ...
                                "country_code": cc,
                                "quality": self._calculate_quality_from_rssi(
                                    int(rssi) if rssi.lstrip("-").isdigit() else -50
                                ),
                                "cipher": "",
                                "authentication": "",
                                "mode": "Master",
    ...
    def analyze_network(self, bssid: str, networks: List[Dict]) -> Dict:
        ...
        if encryption == "OPEN":
            ...
            analysis["attack_vectors"].extend(
                ["WEP key cracking", "ARP replay attacks", "Chopchop attacks"]
            )
            ...
            analysis["attack_vectors"].extend(
                ["WPA handshake capture", "Dictionary attacks", "PMKID attacks"]
            )
            analysis["recommendations"].append("Upgrade to WPA2/WPA3")
            ...
            analysis["attack_vectors"].extend(
                ["WPS PIN attacks (if enabled)", "PMKID attacks", "Brute force attacks"]
            )
            ...
            analysis["attack_vectors"].append(
                "Advanced cryptographic attacks (theoretical)"
            )
...
def test_wifi_scanner():
    ...
    if interfaces:
        ...
        if networks:
            ...
            print(
                f"Sample analysis: {analysis.get('security_assessment', {}).get('level', 'Unknown')}"
            )
