...
class WiFiPumpkinWrapper:
    ...
    def __init__(self):
        ...
        self.wifipumpkin_path = (
            "/Users/morningstar/Desktop/AnonSuite/src/wifi/wifipumpkin3"
        )
        self.python_cmd = "python3"
        self.results_dir = (
            "/Users/morningstar/Desktop/AnonSuite/run/wifipumpkin_results"
        )
    ...
    def check_dependencies(self) -> Dict:
        ...
        if not Path(self.wifipumpkin_path).is_dir():
            status["issues"].append(
                f"WiFiPumpkin3 directory not found: {self.wifipumpkin_path}"
            )
            return status
    ...
    def create_ap_config(
        ...
        config = {
            ...
            "plugins": {"captiveportal": True, "dnsmasq": True, "hostapd": True},
            "logging": {
                "level": "INFO",
                "file": os.path.join(
                    self.results_dir,
                    f"wifipumpkin_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log",
                ),
            },
        }
    ...
    def start_ap(
        ...
        if not all(
            [
                dep_status["wifipumpkin3_available"],
        ...
        try:
            ...
            config_file = self.create_ap_config(
                ssid, interface, channel, security, password
            )
