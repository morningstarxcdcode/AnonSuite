...
from wifipumpkin3.core.common.platforms import Linux, exec_bash
from wifipumpkin3.core.common.terminal import ConsoleUI
from wifipumpkin3.core.controllers.defaultcontroller import DefaultController
...
class PumpkinShell(Qt.QObject, ConsoleUI):
    ...
    def __init__(self, parse_args):
        ...
        if not self.currentSessionID:
            self.currentSessionID = Linux.generate_session_id()
        print(
            display_messages(
                "Session id: {} ".format(
    ...
    def intialize_hook_func(self, controller):
        ...
        for plugin_name in controller.getInfo():
            ...
            if getattr(controller, plugin_name).getPlugins is not None:
    ...
    def do_use(self, args):
        ...
        if args in self.all_modules.keys():
            if module_list()[args].ModPump.getInstance() is not None:
    ...
    def do_start(self, args):
        ...
        ) not in self.interfaces.get("all"):
            print(
                display_messages(
        ...
        if self.wireless_controller.Start() is not None:
            return
        for ctr_name, ctr_instance in self.coreui.getController(None).items():
