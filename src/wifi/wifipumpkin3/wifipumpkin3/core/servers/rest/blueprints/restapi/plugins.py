...
class PluginsInfoResource(Resource):
    ...
    def get(self, plugin_name=None):
        if plugin_name:
            if plugin_name not in self.config.get_all_childname(self.key_name):
                return exception(
...
class SettingsPluginResource(Resource):
    ...
    def get(self, attribute=None):
        if attribute:
            if attribute not in self.config.get_all_childname(self.key_name):
                return exception(
                    "Cannot found that attribute {} on {}!".format(
    ...
    def post(self):
        ...
        for key, value in data.items():
            if key not in self.config.get_all_childname(self.key_name):
                return exception(
            ...
            if str(key).startswith("set_"):
                for item in value:
                    for sub_key, sub_value in item.items():
                        if sub_key not in self.config.get_all_childname(key):
                            return exception(
                                "Cannot found that attribute {} on {}!".format(
                                    sub_key, key
                                ),
