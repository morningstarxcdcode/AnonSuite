...
class ModPump(ModuleUI):
    ...
    def do_add(self, args):
        ...
        if not Linux.check_is_mac(mac_address):
            print(
                display_messages("No valid mac address", error=True)
            )
            return
    ...
    def do_rm(self, args):
        ...
        if mac_address not in self._mac_blacklist:
            print(
                display_messages(
                    "Target MAC address not found", error=True
                )
            )
            return
        ...
        if (
            ...
            if sender in self.aps.keys():
                if Linux.check_is_mac(receiver):
                    if receiver not in self._mac_whitelist:
                        self.aps[sender]["STA"] = {
                            "Frames": 1,
                            "BSSID": sender,
                            "Station": receiver,
                ...
                if Linux.check_is_mac(sender):
                    if sender not in self._mac_whitelist:
    ...
    def showDataOutputScanNetworks(self):
        ...
        for bssid, info in self.aps.items():
            if "(not associated)" not in bssid:
    ...
    def set_monitor_mode(self, mode="manager"):
        if self.options.get("interface")[0] not in Linux.get_interfaces().get("all"):
